...
function seed_demo(): void
{
    ...
    // A separate manager-stage cycle makes submission demonstrable without HR cycle controls.
    $managerCycle=demo_insert('review_cycles',['name'=>'Demo manager review','period_start'=>$day('-90 days'),'period_end'=>$day('-1 day'),'self_deadline'=>$day('-14 days'),'peer_deadline'=>$day('-7 days'),'manager_deadline'=>$day('+14 days'),'status'=>'manager_review','created_by'=>$users['riley']]);
    $ready=demo_insert('review_participants',['cycle_id'=>$managerCycle,'employee_id'=>$users['jamie'],'manager_id'=>$users['casey'],'status'=>'peers_complete']);
    foreach(['jamie'=>'self','alex'=>'peer','drew'=>'peer','blair'=>'peer'] as $person=>$type) {
        $request=demo_insert('feedback_requests',['participant_id'=>$ready,'respondent_id'=>$users[$person],'type'=>$type,'status'=>'submitted','submitted_at'=>date('Y-m-d H:i:s')]);
