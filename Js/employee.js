(function () {
  ...
  function render() {
    ...
    content.innerHTML=`<div class="grid kpis employee-kpis">
      ...
      ${data.reviews.filter(r=>['open','peer_review'].includes(r.cycle_status) && !['manager_submitted','released'].includes(r.status)).map(r=>`<article class="card"><div class="section-head"><div><h3>Peer nominations</h3><p>${esc(r.cycle)}</p></div><button class="btn" data-nominate="${r.id}">Nominate a peer</button></div>${(data.nominations || []).filter(n=>Number(n.participant_id)===Number(r.id)).map(n=>`<p>${esc(n.peer)} · ${label(n.status)}</p>`).join('') || empty('No peers nominated yet.')}</article>`).join('')}
    ...
    content.querySelectorAll('[data-nominate]').forEach(b=>b.onclick=()=>nominateForm(Number(b.dataset.nominate)));
  ...
  async function nominateForm(participantId) {
    try {
      const {candidates}=await request(`peer_candidates&participantId=${participantId}`);
      formDialog('Nominate a peer', `<p>Your manager approves the nomination before your colleague receives a feedback form.</p>
        ${candidates.length?`<form class="workspace-form"><label>Colleague<select name="peer" required><option value="">Choose a colleague</option>${candidates.map(p=>`<option value="${p.id}">${esc(p.name)} · ${esc(p.job_title)}</option>`).join('')}</select></label><button type="submit" class="btn primary">Send nomination</button></form>`:'<p>No eligible colleagues remain for this review.</p>'}`);
      const form=dialog.querySelector('form');
      if(form)form.onsubmit=e=>{e.preventDefault();saveForm(form,'nominate_peer',{participantId,peerId:Number(form.elements.peer.value)});};
