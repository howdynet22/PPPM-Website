(function () {
  ...
  function reviewResultCard(r) {
    ...
    return `<article class="card review-result"><div class="review-result-head"><div><h3>${esc(cycleName(r.cycle))}</h3><p class="muted">Your review results</p></div><span class="review-result-state">${esc(label(r.status))}</span></div>${released
      ? `<div class="review-result-overview"><div class="review-result-rating"><span>Manager’s overall rating</span><strong>${hasRating?rating.toFixed(1):'—'}<small> / 5</small></strong></div><div class="review-result-summary"><strong>Manager summary</strong><p>${r.manager_summary?esc(r.manager_summary):'No summary provided.'}</p></div></div><div class="review-result-peer"><div class="review-result-peer-head"><div><h4>Anonymous peer feedback</h4><p class="muted">Peer averages by competency. These scores are shown separately from the manager’s overall rating. Reviewer identities are hidden.</p></div></div>${scores?`<div class="review-result-scores">${scores}</div>`:`<p class="notice">Anonymous feedback is available after at least ${Number(r.min_peers)} peer responses.</p>`}</div>`
