-- Managers participate in review cycles through Personal while reviewing direct reports in Manager.
...
INSERT IGNORE INTO role_permissions (role_code, permission_id)
SELECT 'leadership', p.id FROM permissions p
...
JOIN users u ON u.is_active = 1 AND u.role = 'manager'
...
WHERE u.role = 'manager' AND rc.status IN ('open', 'peer_review');
